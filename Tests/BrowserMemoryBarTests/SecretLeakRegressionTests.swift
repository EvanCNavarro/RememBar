...
struct SecretLeakRegressionTests {
    ...
    func diagnosticsRedactSensitivePathFields() throws {
        ...
        diagnostics.record(
            "test.path.event",
            fields: [
                "path": "/Users/x/Downloads/recovery-codes.txt",
        ...
        #expect(event.fields["path"]?.contains("recovery-codes.txt") == false)
    ...
    func searchedStatusDisplayDetailRedactsSecretPath() {
        let status = MemorySearchSourceStatus(
            ...
            sourceName: "Files",
            state: .searched,
            detail: "Matched /Users/x/Downloads/recovery-codes.txt"
        )
        #expect(status.displayDetail.contains("recovery-codes.txt") == false)
        #expect(status.accessibilityDetail.contains("recovery-codes.txt") == false)
    }
    ...
    func fileDiagnosticTitleRedacted() {
        ...
        #expect(secret.diagnosticFields["title"]?.contains("recovery-codes.txt") == false)
