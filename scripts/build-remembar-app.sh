...
# Marketing + build version. The release workflow passes REMEMBAR_VERSION=<tag>; a local build with no
# override defaults to the latest shipped version. CFBundleVersion (what Sparkle compares) is derived
# from the version (dots removed) so it rises monotonically across releases — 0.4.2 -> 042 — without a
# second source of truth.
SHORT_VERSION="${REMEMBAR_VERSION:-0.4.2}"
